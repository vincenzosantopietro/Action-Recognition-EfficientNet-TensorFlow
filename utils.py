...
def get_callbacks_list(model_id: int) -> list:
    ...
    log_path = os.path.join(os.getcwd(), "logs", "efficientnetb{}-{}".format(model_id, datetime.now().strftime("%Y%m%d-%H%M%S")))

    if not os.path.exists(model_abs_path):
        os.mkdir(model_abs_path)

    ckpt = tf.keras.callbacks.ModelCheckpoint(filepath=model_abs_path, save_best_only=True)
